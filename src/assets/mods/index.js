...
layui.define(['layer', 'laytpl', 'form', 'element', 'upload', 'util'], function (exports) {
    ...
    content: function (content) {
      ...
      content = fly
        ...
        .replace(/a\([\s\S]+?\)\[[\s\S]*?\]/g, function (str) {
          ...
          if (!href) return str
          const rel = /^(http(s)*:\/\/)\b(?!(\w+\.)*(sentsin.com|layui.com))\b/.test(
            href.replace(/\s/g, '')
          )
          return (
            '<a href="' +
  ...
  form.on('submit(*)', function (data) {
    const action = $(data.form).attr('action')
    const button = $(data.elem)
