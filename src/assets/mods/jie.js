...
layui.define('fly', function (exports) {
  ...
  form.on('select(column)', function (obj) {
    ...
    if (value === '0') {
      ...
      layer.tips(
        '系统会对【分享】类型的帖子予以飞吻奖励，但我们需要审核，通过后方可展示',
        obj.othis,
        tips
      )
    }
  ...
  fly.form['/jie/reply/'] = function (data, required) {
    ...
    laytpl(tpl).render(
      ...
      function (html) {
        ...
        let count = dom.jiedaCount.text() | 0
        ...
        function (res) {
          jieAdmin.append(
            '<span class="layui-btn layui-btn-xs jie-admin ' +
              ...
              (res.data.collection ? 'remove' : 'add') +
              '">' +
              (res.data.collection ? '取消收藏' : '收藏') +
              '</span>'
          )
        }
      )
  ...
  gather.jiedaActive = {
    ...
    reply: function (li) {
      // 回复
      const val = dom.content.val()
      const aite = '@' + li.find('.fly-detail-user cite').text().replace(/\s/g, '')
      dom.content.focus()
  ...
  if (/\/page\//.test(location.href) && !location.hash) {
    const replyTop = $('#flyReply').offset().top - 80
