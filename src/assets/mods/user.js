  ...
  gather.mine = function (index, type, url) {
    ...
    const view = function (res) {
      ...
      elemUCM
        ...
        .eq(index)
        .find('ul')
        .html(res.rows.length === 0 ? '<div class="fly-msg">没有相关数据</div>' : html)
    }

    const page = function (now) {
      const curr = now || 1
  ...
  if ($('#LAY_coop')[0]) {
    ...
    const effectView = function (res) {
      ...
      $('#LAY_effect_count').html(
        '你共有 <strong style="color: #FF5722;">' + (res.count || 0) + '</strong> 笔合作授权订单'
      )
    }

    const effectShow = function (page) {
      fly.json(
        '/cooperation/effect',
        {
          page: page || 1
  ...
  gather.minemsg = function () {
    ...
    */
    ...
    dom.minemsg.on('click', '.mine-msg li .fly-delete', function () {
      const othis = $(this).parents('li')
      const id = othis.data('id')
      fly.json(
        '/message/remove/',
        {
          id
